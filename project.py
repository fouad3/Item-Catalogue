...
# JSON API to view all items for specific category
...
def newCategory():
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Not created! Please add a name to category')
        return redirect(url_for('showCategories'))
    else:
...
def editCategory(category_id):
    ...
    if editedCategory.user_id != login_session['user_id']:
        message = '<script>function myFunction()'
        message += '{alert("You are not authorized to edit this category.'
        message += 'Please create your own category in order to edit.");}'
        message += '</script>'
        message += '<body onload="myFunction()"">'
        return message
    if request.method == 'POST':
        if request.form['name']:
...
def deleteCategory(category_id):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        message = '<script>function myFunction()'
        message += '{alert("You are not authorized to delete this category.'
        message += 'Please create your own category in order to delete.");}'
        message += '</script>'
        message += '<body onload="myFunction()"">'
        return message
    if request.method == 'POST':
        session.query(Item).filter(
            Item.cat_id == category_id).delete(
...
def newItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        message = '<script>function myFunction()'
        message += '{alert("You are not authorized to add items to this '
        message += 'category.Please create your own category in order to add'
        message += 'items.");}</script>'
        message += '<body onload="myFunction()"">'
        return message
    if request.method == 'POST':
        newItem = Item(
...
def editItem(category_id, item_id):
    ...
    if login_session['user_id'] != category.user_id:
        message = '<script>function myFunction()'
        message += '{alert("You are not authorized to edit items to this'
        message += 'category.Please create your own category in order to'
        message += 'edit items.");}</script>'
        message += '<body onload="myFunction()"">'
        return message
    if request.method == 'POST':
        if request.form['name']:
...
# Delete item
@app.route(
    '/category/<int:category_id>/menu/<int:item_id>/delete',
    methods=[
        'GET',
        'POST'])
def deleteItem(category_id, item_id):
    ...
    if login_session['user_id'] != category.user_id:
        message = '<script>function myFunction()'
        message += '{alert("You are not authorized to delete items'
        message += 'to this category.Please create your own category in order'
        message += 'to delete items.");}</script>'
        message += '<body onload="myFunction()"">'
        return message
    if request.method == 'POST':
